...
class ShopifyOrderDataQueueLineEpt(models.Model):
    ...
    def filter_export_stock_queue_lines_and_post_message(self, queues):
        ...
        export_stock_queue_process_cron_time = queues.shopify_instance_id.get_shopify_cron_execution_time(
            "shopify_ept.ir_cron_shopify_auto_export_inventory")
            ...
            for queue_line in self:
                ...
                try:
                    ...
                    if hasattr(error,
                        ...
                        continue
                    message = "Error while Export stock for Product : '%s' for instance:" \
                              "'%s'\nError: %s\n%s" % (odoo_product.display_name, instance.name,
                ...
                #     queue_line.shopify_product_id.write({'last_stock_update_date': datetime.now()})
